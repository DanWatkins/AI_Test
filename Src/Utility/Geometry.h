...
#ifndef _aitest_GEOMETRY_H
#define _aitest_GEOMETRY_H
...
namespace ait
{
	...
	inline double DistanceToRayPlaneIntersection(Vector2D<double> RayOrigin,
												 Vector2D<double> RayHeading,
												 Vector2D<double> PlanePoint,  //any point on the plane
												 Vector2D<double> PlaneNormal)
	{
  
	  double d     = - PlaneNormal.DotProduct(PlanePoint);
	  double numer = PlaneNormal.DotProduct(RayOrigin) + d;
	  double denom = PlaneNormal.DotProduct(RayHeading);
	...
	inline span_type WhereIsPoint(Vector2D<double> point,
								  Vector2D<double> PointOnPlane, //any point on the plane
								  Vector2D<double> PlaneNormal) 
	{
	 Vector2D<double> dir = PointOnPlane - point;

	 double d = dir.DotProduct(PlaneNormal);
	...
	inline double GetRayCircleIntersect(Vector2D<double> RayOrigin,
										Vector2D<double> RayHeading,
										Vector2D<double> CircleOrigin,
										double  radius)
	{
	
	   Vector2D<double> ToCircle = CircleOrigin-RayOrigin;
	   double length      = ToCircle.Length();
	   double v           = ToCircle.DotProduct(RayHeading);
	...
	inline bool DoRayCircleIntersect(Vector2D<double> RayOrigin,
									 Vector2D<double> RayHeading,
									 Vector2D<double> CircleOrigin,
									 double     radius)
	{
	...
	//------------------------------------------------------------------------
	inline bool GetTangentPoints (Vector2D<double> C, double R, Vector2D<double> P, Vector2D<double>& T1, Vector2D<double>& T2)
	{
	  Vector2D<double> PmC = P - C;
	...
	//------------------------------------------------------------------------
	inline double DistToLineSegment(Vector2D<double> A,
									Vector2D<double> B,
									Vector2D<double> P)
	{
	  ...
	  Vector2D<double> Point = A + ((B - A) * dotA)/(dotA + dotB);
	...
	//------------------------------------------------------------------------
	inline double DistToLineSegmentSq(Vector2D<double> A,
									 Vector2D<double> B,
									 Vector2D<double> P)
	{
	  //if the angle is obtuse between PA and AB is obtuse then the closest
	  ...
	  if (dotA <= 0) return A.DistanceToSq(P);

	  //if the angle is obtuse between PB and AB is obtuse then the closest
	  //vertex must be B
	  double dotB = (P.x - B.x)*(A.x - B.x) + (P.y - B.y)*(A.y - B.y);
 
	  if (dotB <= 0) return B.DistanceToSq(P);
    
	  //calculate the point along AB that is the closest to P
	  Vector2D<double> Point = A + ((B - A) * dotA)/(dotA + dotB);

	  //calculate the distance P-Point
	  return P.DistanceToSq(Point);
	}
	...
	inline bool LineIntersection2D(Vector2D<double> A,
								   Vector2D<double> B,
								   Vector2D<double> C, 
								   Vector2D<double> D)
	{
								   ...
								   Vector2D<double>&  point)
	{
		...
		if( (r > 0) && (r < 1) && (s > 0) && (s < 1) )
	  {
		...
		//point = A + r * (B - A);
		//TODO which one is right? Are they both? The top doesn't compile
		point = A + (B-A) * r;
	...
	//------------------------------------------------------------------------
	inline bool ObjectIntersection2D(const std::vector<Vector2D<double>>& object1,
									 const std::vector<Vector2D<double>>& object2)
	{
	...
	//------------------------------------------------------------------------
	inline bool SegmentObjectIntersection2D(const Vector2D<double>& A,
									 const Vector2D<double>& B,
									 const std::vector<Vector2D<double>>& object)
	{
	...
	//  Returns true if the two circles overlap
	//------------------------------------------------------------------------
	inline bool TwoCirclesOverlapped(Vector2D<double> c1, double r1,
							  Vector2D<double> c2, double r2)
	{
	...
	//------------------------------------------------------------------------
	inline bool PointInCircle(Vector2D<double> Pos,
							  double    radius,
							  Vector2D<double> p)
	{
	...
	//------------------------------------------------------------------------
	inline bool   LineSegmentCircleIntersection(Vector2D<double> A,
												Vector2D<double> B,
												Vector2D<double> P,
												double    radius)
	{
	...
	//-----------------------------------------------------------------------------
	inline bool GetLineSegmentCircleClosestIntersectionPoint(Vector2D<double> A,
															 Vector2D<double> B,
															 Vector2D<double> pos,
															 double    radius,
															 Vector2D<double>& IntersectionPoint)
	{
		Vector2D<double> toBNorm = B-A;
		toBNorm.Normalize();

	  //move the circle into the local space defined by the vector B-A with origin
	  //at A
	  Vector2D<double> LocalPos = PointToLocalSpace(pos, toBNorm, toBNorm.Perpendicular(), A);
	  ...
	  if ( (LocalPos.x+radius >= 0) &&
		 ( (LocalPos.x-radius)*(LocalPos.x-radius) <= B.DistanceToSq(A)) )
	  {
