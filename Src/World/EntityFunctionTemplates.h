...
#ifndef _aitest_ENTITY_FUNCTION_TEMPLATES_H
#define _aitest_ENTITY_FUNCTION_TEMPLATES_H
...
namespace ait
{
	...
	void TagNeighbors(const T& entity, conT& ContainerOfEntities, double radius)
	{
		   ...
		   ++curEntity)
	  {
		...
		Vector2D<double> to = (*curEntity)->Pos() - entity->Pos();
										...
										const conT& ContainerOfEntities)
	{
		   ...
		   ++curEntity)
	  {
		...
		Vector2D<double> ToEntity = entity->Pos() - (*curEntity)->Pos();
